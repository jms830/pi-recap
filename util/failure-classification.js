...
export function classifyFailure(err) {
    ...
    if (statusMatch) {
        ...
        if (hasAny(lower, BILLING_MARKERS))
            return `${status} insufficient credits`;
        if (status === "401" || status === "403" || hasAny(lower, AUTH_MARKERS))
            return `${status} auth failed`;
        if (status === "404" || status === "410" || hasAny(lower, RETIRED_MARKERS))
            return `${status} endpoint retired`;
        if (status.startsWith("5") || status === "408")
            return undefined;
        return `${status} ${truncateReason(raw)}`;
    }
    if (hasAny(lower, BILLING_MARKERS))
        return `provider error: ${truncateReason(raw)}`;
    if (hasAny(lower, RETIRED_MARKERS))
        return `provider error: ${truncateReason(raw)}`;
...
const BILLING_MARKERS = ["insufficient", "credits", "payment", "quota"];
const AUTH_MARKERS = ["unauthorized", "forbidden", "invalid api key"];
const RETIRED_MARKERS = ["not found", "retired", "deprecated", "unsupported", "not supported"];
